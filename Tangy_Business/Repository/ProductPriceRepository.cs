...
namespace Tangy_Business.Repository
{
    public class ProductPriceRepository : IProductPriceRepository
    {
        ...
        {
            var obj = _mapper.Map<ProductPriceDTO, ProductPrices>(objDTO);
           
           var addedObj =  _db.ProductPrices.Add(obj);
            await _db.SaveChangesAsync();

           return _mapper.Map<ProductPrices, ProductPriceDTO>(addedObj.Entity);
        }
        ...
        public async Task<ProductPriceDTO> Get(int id)
        {
            ...
            if (obj!=null)
            {
               return _mapper.Map<ProductPrices, ProductPriceDTO>(obj);
            }
        ...
        public async Task<IEnumerable<ProductPriceDTO>> GetAll(int? id =null)
        {
            if (id!=null && id>0)
            {
                return _mapper.Map<IEnumerable<ProductPrices>, IEnumerable<ProductPriceDTO>>
                    (_db.ProductPrices.Where(u=>u.ProductId==id));
            }
            else
            {
                return _mapper.Map<IEnumerable<ProductPrices>, IEnumerable<ProductPriceDTO>>(_db.ProductPrices);
            }
        }
        ...
        public async Task<ProductPriceDTO> Update(ProductPriceDTO objDTO)
        {
            ...
            if (objFromDb!=null)
            {
                ...
                await _db.SaveChangesAsync();
                return _mapper.Map<ProductPrices, ProductPriceDTO>(objFromDb);
            }
            return objDTO;
